...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var yticks = otuIDs.slive(0,10).map(id => "OTU" + id).reverse(); 
    console.log(yticks); 
    ...
    // create a variable that filters the metadata array for the object with the sample 
    var metaData_SelID = data.metadata.filter(data => data.id == sample); 
    console.log(metaData_SelID); 

    // create a variable that holds the washing frequency 
    var washFreguency = metadata_SelID[0].wfreq; 
    ...
    var gaugeLayout = { 
      ...
      annotations: [{ 
        ...
        y: 0, 
        yanchor: "center", 
        text: "the gauge displays belly button washing frequency", 
        showarrow: false
      }]
    }; 
